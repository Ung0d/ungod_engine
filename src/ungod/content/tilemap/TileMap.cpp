...
namespace ungod
{
    ...
    bool TileMap::render(sf::RenderTarget& target, const sf::Texture* tex, sf::RenderStates states)
    {
        ...
        bool active = false;
        if (mVertices.getVertexCount() && tex->getSize().x > 0)
        {
    ...
    bool TileMap::setTiles(TileData& tiles, unsigned mapSizeX, unsigned mapSizeY)
    {
        ...
        mTiles.ids.reset(tiles.ids.release());
        mMapSizeX = mapSizeX;
        mMapSizeY = mapSizeY;
        return true;
    }
    ...
    void TileMap::setTile(int id, unsigned x, unsigned y)
    {
        ...
        if (!mTiles || estimatedPos < 0 || estimatedPos >= mTiles->ids.size())
            return;
    ...
    int TileMap::getTileID(unsigned x, unsigned y) const
    {
        ...
        if (!mTiles || estimatedPos < 0 || estimatedPos >= mTiles->ids.size())
            return -1;
        return *(mTiles.ids)[estimatedPos];
    }
