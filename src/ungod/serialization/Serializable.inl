...
namespace ungod
{
    ...
    template<typename T>
    void SerializationContext::serializeWeak(const std::string& objIdentifier, const T* data, MetaNode serializer)
    {
        if (data)
        {
            auto res = indexMap.emplace(SerialID<T>::get(*data), SerialInfo());
    ...
    BaseSerializable* DeserializationContext::fetch(const EVAL& eval, const std::string& strhash)
    {
        ...
        if (res != typeMap.end() && res->second.nodes.size() > index)
        {
            ...
            if (!nodeinfo.deserializingNow)
            {
                ...
                {
                    nodeinfo.weakQueue.front()(nodeinfo.ptr);
        ...
        {
            Logger::warning("Invalid ref detected.");
            Logger::endl();
            ...
            BaseSerializable* ptr = fetch([this, strhash](NodeInfo& nodeinfo, TypeMapEntry& typeentry)
              {
                  ...
                  {
                    nodeinfo.ptr = typeentry.factory();
                    typeentry.behavior(nodeinfo.ptr, nodeinfo.node, *this);
                  }
                  else
                  {
    ...
    void DeserializationContext::fetchWeak(const std::function<void(T&)>& assigner, const std::string& strhash)
    {
        if (strhash != NULL_ADRESS)
        {
            ...
            std::size_t index;
            try { index = stoi(split.second); } catch (const std::exception& e) { return; }
            auto res = typeMap.find(split.first);
            if (res != typeMap.end() && res->second.nodes.size() < index)
            {
                NodeInfo& nodeinfo = res->second.nodes[index];
                if (nodeinfo.ptr)
                {
                    assigner(*(reinterpret_cast<T*>(nodeinfo.ptr)));
                }
                else  //wait until deserialization takes place
                {
                    nodeinfo.weakQueue.push([assigner, &nodeinfo](BaseSerializable* ptr) { assigner(*(reinterpret_cast<T*>(nodeinfo.ptr))); });
                }
            }
            else
            {
